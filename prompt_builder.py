    ...
    def _format(self):
        ...
        if self._max_length is None:
            return self._format_history(
                self._obs_history[-start_idx - 1 :], self._action_history[-start_idx:]
            )

        for i in reversed(range(1, start_idx + 1)):
            text = self._format_history(
                self._obs_history[-i - 1 :], self._action_history[-i:]
            )
            num_tokens = len(
                text.encode("utf-8")
            )  # Ideal world, we know exactly how many tokens this string is, but estimate using num bytes
