...
def create_texture_map():
    ...
    cell_width, cell_height = map(
        max, zip(*[dummy_draw.textbbox((0, 0), text=chr(i), font=font)[2:] for i in range(256)])
    )
...
def tty_render_image(tty_chars, tty_colors, tty_cursor=None, *, texture_atlas=None):
    ...
    return (
        ...
        .transpose(0, 2, 1, 3, 4)
        .reshape(tty_chars.shape[0] * texture_atlas.shape[1], tty_chars.shape[1] * texture_atlas.shape[2], 3)
    )
...
def tty_render_image_action_history(tty_chars, tty_colors, action_history, tty_cursor=None, *, texture_atlas=None):
    tty_chars_extended = np.pad(
        tty_chars, ((0, 0), (0, MAX_ACTION_LENGTH + 1)), mode="constant", constant_values=ord(" ")
    )
