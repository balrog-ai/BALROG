...
class BabaIsAIWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env, prompt_mode="language", add_ruleset=True):
        super().__init__(env)
    ...
    def get_text_observation(self, obs):
        ...
        other_positions = find_objects(['fball', 'fwall', 'fdoor', "fkey", "rule_object", "rule_is", "rule_property"])
    ...
    def textworld_process_obsv(self, textworld_obsv):
        ...
        if self.prompt_mode == "language":
            return {"text": (prompt, "")}
        # TODO: handle images
        # elif self.prompt_mode == "vision": # or hybrid
        #     return {"rgb_array": (self.env.render(mode="rgb_array"))} 
        else:
