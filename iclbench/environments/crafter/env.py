...
class CrafterLanguageWrapper(crafter.Env):
    ...
    def __init__(self, task="", area=(64, 64), view=(9, 9), size=(64, 64), reward=True, length=10000, seed=None, max_steps=2, vlm=False):
    ...
    def _step_impl(self, action):
        ...
        aug_info['unlocked'] = {name for name, count in self._player.achievements.items() if count > 0 and name not in self._unlocked}
    ...
    def reset(self):
        ...
        return self.process_obs(obs, info, None)
    ...
    def step(self, action):
        ...
        obs = self.process_obs(obs, info, action)
        return obs, reward, done, info
    
    def process_obs(self, obs, info, action):
        img = Image.fromarray(self.render()).convert("RGB") if self.vlm else None
        text = describe_frame(info, action)

        return {
            "text": {"long_term_context": text, "short_term_context": text},
            "image": img,
            "obs": obs,
        }
